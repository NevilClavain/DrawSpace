...
{
    float4x4 mtWorldViewProjection = mat[matWorldViewProjection];
    float4x4 mtWorld = mat[matWorld];
    float4x4 mtView = mat[matView];
    float4x4 mtCam = mat[matCam];
    float4x4 mtProj = mat[matProj];
    
    float4 flags = vec[24];
    float4 cloud_dims = vec[25];
    ...
    float4x4 final_view = mul(mtView, inv);
    ...
    float4x4 world_view = mul(mul(local_trans, mtWorld), final_view);


	// passer le point ( 0, 0, 0 ) du repere cam vers le repere world	
    float4 centerpos2 = mul(centerpos, mtCam);

	// passer le point obtenu du repere world vers le repere local a l'impostor
    float4 centerpos3 = mul(centerpos2, InverseMatrix(mul(local_trans, mtWorld)));

	// il ne reste plus qu'a calculer theta et phi a partir du point obtenu (point 0 camera exprime dans le repere local à l'impostor)
    ...
    Output.Position = mul(mul(mul(vertexpos, mul(rotx, roty)), local_trans), mtWorldViewProjection);
