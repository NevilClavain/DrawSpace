...
float4 bump_bias_vector(float2 p_tex_coords, sampler2D p_sampler, int p_texture_resol, float p_vector_bias)
{
    ...
    float4 bump_left = tex2D(p_sampler, left_coords);
    float4 bump_right = tex2D(p_sampler, right_coords);
    float4 bump_up = tex2D(p_sampler, up_coords);
    float4 bump_down = tex2D(p_sampler, down_coords);

    float bump_center = tex2D(p_sampler, down_coords).x;
