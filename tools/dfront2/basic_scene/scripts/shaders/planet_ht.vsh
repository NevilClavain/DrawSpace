...
VS_OUTPUT vs_main( VS_INPUT Input )
{
	...
	float temperature_alt_dec = 0.0064;  // pente temperature fct de l'altitude : 6.4 ° perdus tout les 1000 metres 
	...
	// pour calcul lattitude du point;

	float h = sqrt( ( v_position2.x * v_position2.x ) + ( v_position2.z * v_position2.z ) );
	...
	temperature_lat = ( ( temperature_max - temperature_min ) * ( 1.0 - ( 2.0 * vertex_latitude / 3.1415927 ) ) ) + temperature_min;
	...
	temperature_final = temperature_alt;
	...
	float humidity_alt_max = 800.0 * pn_humidity_variation + 400.0;
	float res2 = res;
