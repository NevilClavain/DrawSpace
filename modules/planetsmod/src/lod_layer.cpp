...
Layer::Layer( Dynamics::Orbiter* p_owner, Config* p_config, DrawSpace::Dynamics::World* p_world, LOD::Body* p_body, 
    ...
    if( m_collisions )
    {
        ...
        if( m_current_patch && p_patch_lod == 0 )
        {
            ...
            if( p_patch->GetOrientation() == m_body->GetCurrentFace() )
            {
                ...
                FaceDrawingNode* node = static_cast<FaceDrawingNode*>( m_current_collisions_hm->GetNode() );
...
void Layer::build_meshe( DrawSpace::Core::Meshe& p_patchmeshe, Patch* p_patch, DrawSpace::Core::Meshe& p_outmeshe, float* p_heightmap )
{
...
void Layer::Compute( Root* p_owner )
{
    ...
    {        
        if( m_hot )
        {
            m_body->UpdateHotPoint( m_relative_hotviewerpos ); // m_relative_hotviewerpos est mis � jour seulement 
...
CameraPoint* Layer::GetCamera( void ) const
{
    return m_camera;
}

InertBody* Layer::GetInertBody( void ) const
{
    return m_inertbody;
}
...
LOD::Body* Layer::GetBody( void ) const
{
    return m_body;
}

void Layer::GetCollisionMesheBuildStats( long& p_nb_collisionmeshebuild_done ) const
{
...
void Layer::SubPassDone( Collisions* p_collider )
{
...
dsreal Layer::GetLastMaxHeight( void ) const
{
    return m_currentpatch_max_height;
}

dsreal Layer::GetCurrentHeight( void ) const
{
    return m_currentpatch_current_height;
}

LOD::Body* Layer::GetSlodBody( void ) const
{
    return m_body;
}

DrawSpace::Dynamics::Collider* Layer::GetCollider( void ) const
{
    return m_collider;
}
