...
Root::Root( const dsstring& p_scenename, dsreal p_ray, DrawSpace::Utils::TimeManager* p_time, const Config& p_config ) : Orbiter( &m_world ),
...
m_enable_gravity( true )
{
    ...
    m_config = _DRAWSPACE_NEW_( Config, Config );
    *m_config = p_config;
       
    m_drawable = _DRAWSPACE_NEW_( Drawing, Drawing( m_config ) );
    m_drawable->SetRenderer( SingletonPlugin<DrawSpace::Interface::Renderer>::GetInstance()->m_interface );
    
    m_camera_evt_cb = _DRAWSPACE_NEW_( CameraEvtCb, CameraEvtCb( this, &Root::on_camera_event ) );
    m_nodes_evt_cb = _DRAWSPACE_NEW_( NodesEventCb, NodesEventCb( this, &Root::on_nodes_event ) );
    m_scenegraph_evt_cb = _DRAWSPACE_NEW_( ScenegraphEventCb, ScenegraphEventCb( this, &Root::on_scenegraph_event ) );

    m_subpass_creation_cb = _DRAWSPACE_NEW_( SubPassCreationCb, SubPassCreationCb( this, &Root::on_subpasscreation ) );

    m_timer_cb = _DRAWSPACE_NEW_( TimerCb, TimerCb( this, &Root::on_timer ) );
...
Root::~Root( void )
{
    ...
    for( size_t i = 0; i < m_layers_list.size(); i++ )
    {
        ...
        LOD::Body* slod_body = m_layers_list[i]->GetSlodBody();
...
void Root::on_camera_event( DrawSpace::Core::SceneNodeGraph::CameraEvent p_event, DrawSpace::Core::BaseSceneNode* p_node )
{
    if( DrawSpace::Core::SceneNodeGraph::ACTIVE == p_event )
    {
        ...
        if( m_registered_camerapoints.count( current_camera_scenename ) > 0 )
        {
            ...
            {
                LOD::Layer* layer = m_registered_camerapoints[current_camera_scenename].layers[i];
        ...
        if( inertbody )
        {
            ...
            if( inertbody->IsDynamicLinkEnabled() )
            {
                ...
                if( inertbody->IsDynamicLinkInitState() )
                {
                    if( inertbody->GetReferentBody() == this )
                    {
                        ...
                        {                                                                                                                
                            ...
                            LOD::Body* slod_body = _DRAWSPACE_NEW_( LOD::Body, 
                                LOD::Body( m_timemanager, m_config, i, m_subpass_creation_cb, m_config->m_nbLODRanges_inertBodies ) );

                            Collider* collider = _DRAWSPACE_NEW_( Collider, Collider );
                        
                            LOD::Layer* layer = _DRAWSPACE_NEW_( LOD::Layer, LOD::Layer( this, m_config, &m_world, slod_body, 
                ...
                else
                {
                    ...
                    for( size_t i = 0; i < m_config->m_layers_descr.size(); i++ )
                    {                                                                                                                
                        LOD::Body* slod_body = _DRAWSPACE_NEW_( LOD::Body, LOD::Body( m_timemanager, m_config, i, m_subpass_creation_cb, m_config->m_nbLODRanges_inertBodies ) );

                        Collider* collider = _DRAWSPACE_NEW_( Collider, Collider );
                   
                        Layer* layer = _DRAWSPACE_NEW_( Layer, Layer( this, m_config, &m_world, slod_body, collider, m_subpass_creation_cb, i ) );
...
void Root::create_camera_collisions( const dsstring& p_cameraname, CameraPoint* p_camera, Root::RegisteredCamera& p_cameradescr, bool p_hotstate )
{
    for( size_t i = 0; i < m_config->m_layers_descr.size(); i++ )
    {
        LOD::Body* slod_body = _DRAWSPACE_NEW_( LOD::Body, LOD::Body( m_timemanager, m_config, i, m_subpass_creation_cb, m_config->m_nbLODRanges_freeCameras ) );
...
void Root::on_timer( DrawSpace::Utils::Timer* p_timer )
{
    ...
    {
        SubPass* pass = m_singleshot_subpasses_stack.back();
...
void Root::manage_bodies( void )
{
    for( std::map<DrawSpace::Dynamics::InertBody*, RegisteredBody>::iterator it = m_registered_bodies.begin(); it != m_registered_bodies.end(); ++it )
    {
        ...
        for( size_t i = 0; i < it->second.layers.size(); i++ )
        {
            ...
            {
                Layer* layer = it->second.layers[i];
...
void Root::RegisterSinglePassSlot( Pass* p_pass, Binder* p_binder, int p_orientation, LOD::Body::MesheType p_meshe_type, int p_layer_index, int p_rendering_order )
{
...
LOD::Layer* Root::GetLayerFromInertBody( DrawSpace::Dynamics::InertBody* p_body, int p_layer_index )
{
...
Layer* Root::GetLayerFromCamera( const dsstring& p_cameraName, int p_layer_index )
{
...
void Root::DrawSubPasses( void )
{
    if( m_singleshot_subpasses_stack.size() > 0 )
    {
        SubPass* sp = m_singleshot_subpasses_stack.back();
    ...
    {
        SubPass* sp = m_permanent_subpasses[i];        
        sp->DrawSubPass();
        sp->SubPassDone();
    }
}

SubPass::EntryInfos Root::on_subpasscreation( SubPass* p_pass, int p_dest )
{
    // cb traitement d'une requete de creation d'une sub-pass
 
    FaceDrawingNode* node = static_cast<FaceDrawingNode*>( p_pass->GetNode() );
