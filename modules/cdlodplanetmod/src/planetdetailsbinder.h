...
public:
	...
	dsreal GetOceansDetailsAlt( void) const { return m_ocean_details_alt; };

	PlanetLight GetLight( int p_index ) const { return m_lights[p_index]; };

    void SetLandscapeBumpFactor( dsreal p_factor );

    dsreal GetAtmoThickness( void ) const { return m_atmoThickness; };
