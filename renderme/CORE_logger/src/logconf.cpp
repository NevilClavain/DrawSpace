...
void logger::Configuration::on_new_line( const std::string& p_line, long p_line_num, const std::vector<std::string>& p_words )
{
    if( "logger" == p_words[0] )
    {
        if( p_words.size() < 5 )
        {
            _EXCEPTION("log configuration file : missing params for sink, line " + std::to_string(p_line_num));
        }

        if( 0 == Configuration::getInstance()->m_outputs.count( p_words[4] ) )
        {
            _EXCEPTION("log configuration file : unknown output name for sink, line " + std::to_string(p_line_num));
        }
        ...
        {
            _EXCEPTION("log configuration file : bad state for sink, line " + std::to_string(p_line_num));
        }

        static const std::map<std::string, Sink::Level> str_to_level =
        {
    ...
    else if( "output" == p_words[0] )
    {
        if( p_words.size() < 5 )
        {
            _EXCEPTION("log configuration file : missing params for output, line " + std::to_string(p_line_num));
        }
        ...
        {
            _EXCEPTION("log configuration file : unknown class for output, line  " + std::to_string(p_line_num));
        }
    }
}
