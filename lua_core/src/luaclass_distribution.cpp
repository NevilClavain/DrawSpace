...
{    
    ...
    if("uniform_int_distribution" == distribution_type)
    {
        ...
        const auto min{ luaL_checkint(p_L, 2) };
        const auto max{ luaL_checkint(p_L, 3) };
    ...
    else if("uniform_real_distribution" == distribution_type)
    {
        ...
        const auto min{ luaL_checknumber(p_L, 2) };
        const auto max{ luaL_checknumber(p_L, 3) };
...
int LuaClass_Distribution::LUA_generate(lua_State* p_L)
{
    ...
    const auto lua_random_engine{ Luna<LuaClass_RandomEngine>::check(p_L, 1) };

    if("uniform_int_distribution" == m_distribution_type)
    {
        auto dw{ m_distribution.get() };
        const auto distribution{ static_cast<DistributionWrapperImpl<std::uniform_int_distribution<int>>*>(dw) };

        const auto val{ distribution->Generate<int>(lua_random_engine->GetRandomEngine()) };
    ...
    {
        auto dw{ m_distribution.get() };
        const auto distribution{ static_cast<DistributionWrapperImpl<std::uniform_real_distribution<dsreal>>*>(dw) };

        const auto val{ distribution->Generate<dsreal>(lua_random_engine->GetRandomEngine()) };
